...
public class Main {
    public static void main(String[] args) {
        ...
        Room oxford1 = new Room("Oxford", "Guest Room", 5, 225.00);
        Room victoria = new Room("Victoria", "Suite", 5, 225.00);

        List<Room> rooms = new ArrayList<>(List.of(piccadilly, cambridge, oxford, oxford1, victoria));

        //Collections.sort(rooms, Comparator.naturalOrder());
        rooms.sort(Room.RATE_COMPARATOR);

        Room newLondon = new Room("New London", "Suite", 5, 225.00);
        ...
        System.out.format("%n%n--List Contents--%n%n");
        rooms.stream()
